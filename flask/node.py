...
def add_transaction():
    ...
    if not all(field for field in required_field):
        response = {
...
def mine():
    ...
    if block != None:
        ...
        response = {
            ...
            'block': dict_block,
            'current_funds': blockchain.get_balance()
        }
        return jsonify(response), 201
    else:
        response = {
...
def remove_node(node_url):
    ...
    """else:
        ...
        response  ={
            'message': 'Node removed sucessully',
            'all_nodes': blockchain.get_peer_nodes()
        }
        return jsonify(response), 200"""
    if blockchain.remove_peer_nodes(node_url) == True:
        response = {
            'message': 'Node removed sucessully',
            'all_nodes': blockchain.get_peer_nodes()
        }
        return jsonify(response), 200
    elif blockchain.remove_peer_nodes(node_url) == False:
        response = {
